  ...
  return (
    <div ref={projectRef} className={`flex flex-col-reverse lg:flex-row my-10 lg:my-20 gap-10 lg:gap-4 fadeInUp ${isEven ? 'lg:flex-row-reverse' : ''}`}>
      ...
      <div className={`flex flex-col justify-between ${!isEven ? 'text-right items-end' : 'text-left'}`}>
        <div>
          <p className={`text-sky-400 text-xs lg:text-sm ${fonts.inter.className}`}>Featured Project</p>
          <h2 className={`text-lg lg:text-2xl font-semibold mb-2 ${fonts.poppins.className}`}>{project.title}</h2>
          <p className={`mb-4 text-gray-500 dark:text-gray-300 rounded-sm text-sm lg:text-base p-3 bg-gray-300 dark:bg-gray-800 shadow-md ${fonts.inter.className}`}>
          {project.description}
          </p>
        
          <ul className={`list-none text-sm lg:text-base flex gap-1 md:gap-3 text-sky-400 ${!isEven ? 'justify-end' : 'justify-start'} ${fonts.inter.className}`}>
