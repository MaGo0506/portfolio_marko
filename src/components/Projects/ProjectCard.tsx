  ...
  return (
    <div className={`flex my-20 gap-4 ${isEven ? 'flex-row-reverse' : ''}`}>
      <div className="flex-shrink-0 relative">
        ...
        <a href={project.link} target="_blank" className="absolute top-0 left-0 h-full w-full z-10 bg-sky-400 opacity-70 hover:opacity-0 transition-opacity duration-200 rounded-md"/>
      </div>
      ...
      <div className={`flex flex-col justify-between ${!isEven ? 'text-right items-end' : 'text-left'}`}>
        ...
        <p className="mb-4 text-gray-500 rounded-sm dark:text-gray-300 p-3 bg-gray-300 dark:bg-gray-800 shadow-md">{project.description}</p>
